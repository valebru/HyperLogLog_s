...
--[[
		file:write(("%s%s%s: %.2f Mpps, %.0f Mbit/s\n"):format(
			getColorCode(colors[direction]), direction, getColorCode(),
			mpps, mbit
		))
]]--
--[[
...
local function getPlainFinal(direction)
	return function(stats, file)
		file:write(("%s: %.2f (%.2f) Mpps, %.0f (%.0f) Mbit/s | %d pkts, %d Bytes\n"):format(
			direction,
...
--[[
		file:write(("%s%s%s: %.2f (%.2f) Mpps, %.0f (%.0f) Mbit/s | %d pkts, %d Bytes\n"):format(
			getColorCode(colors[direction]), direction, getColorCode(),
			stats.mpps.avg, stats.mpps.stdDev,
			stats.mbit.avg, stats.mbit.stdDev,
			stats.total, stats.totalBytes
...
--	end
	...
	io.write("[RX - dev]: { ")
	for i = #args.rxDevices, 1, -1 do
		local id = args.rxDevices[i].id or args.rxDevices[i].dev.id
		io.write(tonumber(id) .. " ")
	end
	io.write("} \n")

	io.write("[TX - dev]: { ")
	for i = #args.rxDevices, 1, -1 do
		local id = args.txDevices[i].id or args.txDevices[i].dev.id
		io.write(tonumber(id) .. " ")
	end
	io.write("} \n")
...
local function ValeStatsTask(args)
	print("==========VVV=============")
	local counters = {}
...
__LIBMOON_STATS_TASK = ValeStatsTask
